...
/**
 * is_avl - checks if a valid (NON-NULL) tree is a AVL.
...
{
	int avl_flag = 1, min_right, max_left;
	...
	avl_flag &= is_avl(tree->left);
	avl_flag &= is_avl(tree->right);
	return (avl_flag);
